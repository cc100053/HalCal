...
- `UsageGuideSheet` is written in its own fixed 1060 x 580 design space and fitted to the turned frame by scaling `LocalDensity`, not by scaling a graphics layer over a `requiredSize`. A required size overflows the ~393dp-tall frame by ~94dp at each end, and the overflowing sheet draws clipped to the frame for its first frames, so the header and the footer arrive about 200ms after the middle of the sheet. Scaling the density makes the laid-out size the drawn size, so nothing overflows and there is nothing to clip.
...
- Respect the haptic preference toggle when adding interactions. It is the only setting left; the bead sound effect and its toggle have been removed.
