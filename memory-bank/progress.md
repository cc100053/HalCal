...
All pass under `./gradlew test` as of 2026-07-16, including Japanese calculator error display coverage.

`assembleDebug` and `lint` also pass as of 2026-07-16. Emulator visual checks covered phone/tablet sizing, portrait/landscape orientation, light/dark themes, calculator, tax, settings, and soroban surfaces. A fresh API 36 Pixel 7 check under an `en-US` device locale confirmed that the app still renders Japanese-only labels without clipping.
